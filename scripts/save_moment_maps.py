...
def save_to_fits(ra,
    ...
    if 3600*min([hdr_in["BMAJ"],hdr_in["BMIN"]])<0.99*target_res:
        ...
        map_cartesian, _ = reproject_interp((map_cartesian,hdr_in), hdr_in_repr)
        hdr_in = hdr_in_repr
        
    # save fits file
    fname_fits = folder + f'{this_source}_{line}_{filename}.fits'
    fits.writeto(fname_fits, data=map_cartesian, header=hdr_in, overwrite=True)
...
def save_mom_to_fits(fname,
                     ...
                     target_res):
    """
    Function to prepare and convert the moment maps created on a hexagonal grid onto a 
    cartesian one and save as FITS file.
    ...
    for ii, this_source in enumerate(source_list):
        ...
        for line in lines_data["line_name"]:
            # Mom0
            save_to_fits(ra_deg, dec_deg, target_hdr_list[ii], target_slice, 
                         "MOM0", "mom0", this_source, this_data, line, folder, target_res)
            save_to_fits(ra_deg, dec_deg, target_hdr_list[ii], target_slice, 
                         "EMOM0", "emom0", this_source, this_data, line, folder, target_res)
            # Mom1
            save_to_fits(ra_deg, dec_deg, target_hdr_list[ii], target_slice, 
                         "MOM1", "mom1", this_source, this_data, line, folder, target_res)
            save_to_fits(ra_deg, dec_deg, target_hdr_list[ii], target_slice, 
                         "EMOM1", "emom1", this_source, this_data, line, folder, target_res)
            # Mom2
            save_to_fits(ra_deg, dec_deg, target_hdr_list[ii], target_slice, 
                         "MOM2", "mom2", this_source, this_data, line, folder, target_res)
            save_to_fits(ra_deg, dec_deg, target_hdr_list[ii], target_slice, 
                         "EMOM2", "emom2", this_source, this_data, line, folder, target_res)
            # Tpeak and rms
            save_to_fits(ra_deg, dec_deg, target_hdr_list[ii], target_slice, 
                         "TPEAK", "tpeak", this_source, this_data, line, folder, target_res)
            save_to_fits(ra_deg, dec_deg, target_hdr_list[ii], target_slice, 
                         "RMS", "rms", this_source, this_data, line, folder, target_res)
            

def save_band_to_fits(fname,
                      bands_data,
                      source_list,
                      run_success,
                      target_hdr_list, 
                      ...
                      target_res):
    """
    Function to prepare and convert the band maps created on a hexagonal grid onto a 
    cartesian one and save as FITS file.
    :param fname: list of string with PyStructure file names
    :param lines_data: PD Database with information about individual spectral lines
    :param source_list: List of Strings; name of individual sources
    ...
    for ii, this_source in enumerate(source_list):
        ...
        # iterate over bands
        for band in bands_data["band_name"]:
            save_to_fits(ra_deg,dec_deg, target_hdr_list[ii], target_slice, 
                         "BAND", "map", this_source, this_data, band, folder, target_res)
            save_to_fits(ra_deg,dec_deg, target_hdr_list[ii], target_slice, 
                         "EBAND", "emap", this_source, this_data, band, folder, target_res)
